    ...
    <div class="sm:flex mx-5 sm:mx-8 lg:mx-10 xl:mx-20">
        <div>
            <h1 class="font-bold text-lg lg:text-xl xl:text-2xl">
                Petunjuk Ujian {{$ujian['ujian']->judul}}
            </h1>
            <h1
...
<section class="text-black mt-8 xl:mt-12 mb-10">
    ...
    >
        ...
        >
            ...
            <div class="ml-3">
                <p>{{ abs(\Carbon\Carbon::createFromFormat('H:i:s', $ujian['ujian']->durasi)->diffInMinutes(\Carbon\Carbon::createFromFormat('H:i:s', '00:00:00'))) }} menit</p>
                <p class="mt-1">{{ \Carbon\Carbon::parse($ujian['ujian']->deadline)->format('j F Y, \p\u\k\u\l H:i') }}</p>
            </div>
        </div>
        <p
            class="mt-3 sm:mt-4 lg:mt-5 text-[12.5px] sm:text-[12.5px] md:text-[13px] lg:text-sm xl:text-[15px]"
        >
            {!! $ujian['ujian']->deskripsi !!}
        </p>
        <div class="flex mt-5 md:mt-7 lg:mt-8">
            ...
            @if($ujian['pengerjaanSiswa']->status != "dikerjakan")
                <a href="{{route("begin-siswa-examination", ['mapelId' => $mapel['mapel_id'], 'ujianId' => $ujian['ujian']->id, 'siswaId' => $siswa->id])}}" class="ml-auto">
                    ...
                    >
                        <p
                            class="text-white text-[12px] md:text-[12.5px] font-semibold lg:text-[13px] xl:text-[14px]"
                        >
                            Mulai Ujian
                        </p>
                    </div>

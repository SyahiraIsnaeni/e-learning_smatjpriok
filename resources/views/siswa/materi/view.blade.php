...
<section class="text-black mt-8 mb-10 xl:mt-10">
    <div class="mx-5 sm:mx-8 lg:mx-10 xl:mx-20">
        @foreach($materi as $row)
            <a href="{{route("detail-siswa-material", ["mapelId" => $mapel["mapel_id"], "materiId" => $row->id, "siswaId" => $siswa->id])}}">
                <div
                    class="bg-white mt-2.5 lg:mt-3 flex bg-opacity-90 shadow px-3 py-3 lg:py-4 rounded-lg border border-black border-opacity-25 transition ease-in-out hover:-translate-y-1.5 duration-300 hover:shadow-md"
                >
                    <div class="sm:w-2/3 mr-2 sm:mr-5 ml-2 md:ml-3 lg:ml-5 w-full">
                        ...
                        <div class="sm:hidden flex">
                            <div class="w-3/4 mr-5">
                                <h1
                                    class="font-bold text-[13px] sm:text-sm md:text-[15.5px] lg:text-base"
                                >
                                    {{ strlen(strip_tags($row->judul)) > 50 ? substr(strip_tags($row->judul), 0, 50) . '...' : strip_tags($row->judul) }}
                                </h1>
                                <p class="font-normal text-xs lg:text-[13px] mt-1 lg:mt-1.5">
                                    {{$row->created_at->format('j F Y')}}
                                </p>
                            </div>
                            @if($row->is_read == 0)
                                <div class="flex justify-center ml-auto mt-1 w-1/4">
                                    <div class="rounded-full w-[7px] h-[7px] bg-[#A33F46] mt-[5px]"></div>
                                    <p class="text-[#A33F46] text-xs font-semibold ml-1">Belum Dibaca</p>
                                </div>
                            @else
                                <div class="flex justify-center ml-auto mt-1 w-1/4">
                                    <svg xmlns="http://www.w3.org/2000/svg" height="11" width="11" viewBox="0 0 448 512" class="mt-[2.5px]" fill="#008958" stroke="#008958" stroke-width="40">
                                        <path d="M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7 393.4 105.4c12.5-12.5 32.8-12.5 45.3 0z"/>
                                    </svg>
                                    <p class="text-[#008958] text-xs lg:text-[13px] font-semibold ml-[2px] lg:ml-[4px]">Sudah Dibaca</p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="hidden sm:block">
                            <h1 class="font-bold text-[15px] md:text-[15.5px] lg:text-base">
                                {{$row->judul}}
                            </h1>
                            <p class="font-normal text-xs lg:text-[13px] mt-1 lg:mt-1.5">
                                {{$row->created_at->format('j F Y')}}

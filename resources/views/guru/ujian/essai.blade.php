...
<section class="text-black mt-8 xl:mt-12 mb-10">
    <div class="mx-5 sm:mx-8 lg:mx-10 xl:mx-20">
        <form action="{{ route("course-guru-add-data-essai", ['mapelId' => $mapel['mapel_id'], 'guruId' => $guru->id]) }}" method="post" enctype="multipart/form-data">
            @csrf
            <label for="judul" class="lg:flex">
                ...
                <div class="lg:w-3/4">
                    <input
                        class="mt-1.5 xl:mt-2 px-3 py-2 xl:py-3 mr-4 border-2 rounded-md w-full block text-[12.5px] lg:text-sm xl:text-[15px] lg:placeholder:text-[12.5px] xl:placeholder:text-sm"
                        type="text"
                        id="judul"
                        name="judul"
            ...
            <label for="deskripsi" class="lg:flex mt-5">
                ...
                <div class="form-group lg:w-3/4" style="margin-top: 10px">
                    <textarea id="inp_editor0" name="deskripsi">
                                                 &lt;p&gt;&lt;/p&gt;
                                                </textarea>
                    <script>
                        var editor1 = new RichTextEditor("#inp_editor0");
                    </script>
                </div>
            </label>
            ...
            <label for="deadline" class="lg:flex mt-5">
                <div class="lg:w-1/4">
                    <p
                        class="text-[13.5px] lg:text-[15px] xl:text-base font-medium mt-4 xl:mt-4"
                    >
                        Tenggat/Deadline
                    </p>
                </div>
            ...
            <label for="durasi" class="lg:flex mt-5">
                <div class="lg:w-1/4">
                    <p
                        class="text-[13.5px] lg:text-[15px] xl:text-base font-medium mt-4 xl:mt-4"
                    >
                ...
                <div class="md:w-[300px]">
                    <input
                        class="mt-1.5 xl:mt-1 px-3 py-2 lg:py-2.5 mr-4 border-2 rounded-md w-full block text-[12.5px] lg:text-sm xl:text-[15px] lg:placeholder:text-[13.5px] xl:placeholder:text-sm"
                        type="time"
                        id="durasi"
                        name="durasi"
                        step="1"
            ...
            <label for="pertanyaan" class="mt-5">
                <p class="text-[13.5px] lg:text-[15px] xl:text-base font-medium mt-5 xl:mt-4">Soal Ujian</p>
                <div class="container bg-white rounded-md w-full border shadow mt-1.5 py-4 px-3 relative" id="soalContainer">
                    <!-- SOAL ESSAY -->
                    <div class="soal bg-[#E6F4F1] rounded-md py-3 px-3 border-2" id="examinationSection1">
                        <div class="flex">
                            <div class="w-5/6">
                                <input type="hidden" name="nomor[]" value="Soal 1">
                                <p class="text-[13px] lg:text-sm xl:text-[15px] font-bold mt-1.5">Soal 1</p>
                            </div>
                        </div>
                        <div class="form-group lg:w-full" style="margin-top: 10px">
                    <textarea id="inp_editor1" name="pertanyaan[]">
                                                 &lt;p&gt;&lt;/p&gt;
                                                </textarea>
                            <script>
                        ...
                        <div class="flex mt-2">
                            <button class="bg-[#EE982B] py-2 px-3 lg:px-3.5 lg:py-2.5 xl:px-4 xl:py-3 rounded-md mt-3 block mr-auto transition ease-in-out hover:scale-105 duration-200 cancel-btn text-xs lg:text-[13px] xl:text-[13.5px] font-semibold text-black" onclick="cancelSoal(event)">Cancel</button>
                        </div>
                    </div>
                </div>
            </label>
            ...
            <script>
                ...
                function tambahSoal() {
                    ...
                    newSoalDiv.classList.add("soal", "bg-[#E6F4F1]", "rounded-md", "py-3", "px-3", "border-2", "mt-4");
                    newSoalDiv.id = "examinationSection" + soalCounter;

                    const newSoalContent = `
                        <div class="flex">
                            <div class="w-5/6">
                                <input type="hidden" name="nomor[]" value="Soal ${soalCounter}">
                                <p class="text-[13px] lg:text-sm xl:text-[15px] font-bold mt-1.5">Soal ${soalCounter}</p>
                            </div>
                        </div>
                        <div class="form-group lg:w-full" style="margin-top: 15px">
                            <textarea id="inp_editor${soalCounter}" name="pertanyaan[]"></textarea>
                        </div>
                        <div class="flex mt-2">
                            <button class="bg-[#EE982B] py-2 px-3 lg:px-3.5 lg:py-2.5 xl:px-4 xl:py-3 rounded-md mt-3 block mr-auto transition ease-in-out hover:scale-105 duration-200 cancel-btn text-xs lg:text-[13px] xl:text-[13.5px] font-semibold text-black" onclick="cancelSoal(event)">Cancel</button>
                        </div>
                    ...
                    newSoalDiv.innerHTML = newSoalContent;
                    soalContainer.appendChild(newSoalDiv);

                    // Menambahkan script untuk editor teks
                    const script = document.createElement("script");
                    script.textContent = `var editor${soalCounter} = new RichTextEditor("#inp_editor${soalCounter}");`;
                    newSoalDiv.appendChild(script);

                    // Memperbarui nomor soal untuk semua soal yang ada
                    const allSoalDivs = document.querySelectorAll(".soal");
                    allSoalDivs.forEach((soalDiv, index) => {
                        // Mendapatkan elemen input nomor soal
                        const nomorInput = soalDiv.querySelector("input[name='nomor[]']");
                        // Memperbarui nilai nomor soal
                        nomorInput.value = `Soal ${index + 1}`;
                        // Memperbarui teks nomor soal
                        const nomorText = soalDiv.querySelector(".font-bold");
                        nomorText.innerText = `Soal ${index + 1}`;
                    });
                }

                function cancelSoal(event) {
                    const soalDiv = event.target.closest(".soal");
                    soalDiv.remove();

                    // Memperbarui nomor soal untuk semua soal yang tersisa
                    const allSoalDivs = document.querySelectorAll(".soal");
                    allSoalDivs.forEach((soalDiv, index) => {
                        // Mendapatkan elemen input nomor soal
                        const nomorInput = soalDiv.querySelector("input[name='nomor[]']");
            ...
            <!-- TOMBOL TAMBAH SOAL -->
            <div class="flex mt-7 pb-10 mb-3">
                <div class="flex">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 xl:h-[17px] xl:w-[17px] xl:mt-0.5" viewBox="0 0 512 512">
                        <path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM232 344V280H168c-13.3 0-24-10.7-24-24s10.7-24 24-24h64V168c0-13.3 10.7-24 24-24s24 10.7 24 24v64h64c13.3 0 24 10.7 24 24s-10.7 24-24 24H280v64c0 13.3-10.7 24-24 24s-24-10.7-24-24z"/>
                    </svg>
                    <a href="#" id="tambahSoalBtn">
                        <p class="text-xs lg:text-[13px] xl:text-sm underline font-semibold ml-1 hover:font-bold" onclick="tambahSoal()">Tambah Soal</p>
                    </a>
                </div>
            </div>
            <button

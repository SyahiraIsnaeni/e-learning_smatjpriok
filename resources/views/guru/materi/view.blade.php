...
<section class="mt-8 xl:mt-12">
    <div class="mx-5 sm:mx-8 lg:mx-10 xl:mx-20 flex">
        <a href="{{route("add-guru-material", ['mapelId' => $mapel['mapel_id'], 'guruId' => $guru->id])}}">
            <div class="bg-[#763500] rounded-md px-3 py-2.5 xl:py-3 lg:px-4 flex transition ease-in-out hover:scale-105 duration-300">
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    height="16"
                    width="14"
                ...
                <p class="ml-1.5 text-xs lg:text-[13px] xl:text-sm font-semibold text-white">
                    Tambah Materi
                </p>
            </div>
        </a>
...
<section class="text-black mt-5 mb-10">
    <div class="mx-5 sm:mx-8 lg:mx-10 xl:mx-20">
        ...
        <a href="{{route('detail-guru-material', ['mapelId' => $mapel['mapel_id'], 'materiId' => $row->id, 'guruId' => $guru->id])}}">
            <div class="relative bg-white-first mt-3">
                <div
                ...
                >
                    <a href="{{route('edit-guru-material', ['mapelId' => $mapel['mapel_id'], 'materiId' => $row->id, 'guruId' => $guru->id])}}">
                        <li class="mb-2 px-4 hover:font-bold">Edit</li>
                    </a>
                    <form method="post" action="{{route('delete-guru-data-material', ['mapelId' => $mapel['mapel_id'], 'materiId' => $row->id, 'guruId' => $guru->id])}}" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button >
            ...
            <script>
                function showOptions(event, materiId) {
                    event.stopPropagation();
                    event.preventDefault();
                    const optionsList = document.getElementById("optionsList"+materiId);

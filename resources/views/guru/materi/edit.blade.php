...
<section class="text-black mt-8 xl:mt-12 mb-10">
    <div class="mx-5 sm:mx-8 lg:mx-10 xl:mx-20">
        <form method="post" action="{{ route('edit-guru-data-material', ['mapelId' => $mapel['mapel_id'], 'materiId' => $materi->id, 'guruId' => $guru->id])}}" enctype="multipart/form-data">
            @csrf
            ...
            <label for="gambar" class="lg:flex mt-5 lg:mt-8">
                <div class="lg:w-1/4">
                    <p class="text-[13.5px] lg:text-[15px] xl:text-base font-medium mt-3 lg:mt-0">
                        Gambar (opsional)
                    </p>
                </div>
                <div class="lg:w-3/4 items-center">
                    <input class="text-[12.5px] mt-1.5 lg:mt-0" type="file" id="gambar" name="gambar" accept="image/*">

                    <p  class="text-xs md:text-[13px] lg:text-sm mt-4">Gambar Saat Ini</p>
                    @if ($materi->gambar != null)
                        <img src="{{ asset('storage/public/materi/gambar/' . $materi->gambar) }}" alt="Gambar Saat Ini" class="h-20 w-20 object-cover mt-1">
                    @else
                        <span class="text-sm text-gray-400 mt-1">Tidak ada gambar saat ini</span>
                    @endif
                </div>
            </label>
